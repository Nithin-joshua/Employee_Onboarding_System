...
describe('Employee Onboarding Workflow (e2e)', () => {
  ...
  beforeEach(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .useValue({
        ...
        getSignedUrl: jest.fn().mockResolvedValue('https://mock-signed-url.com/doc.pdf'),
      })
  ...
  it('Happy Path - salary < 21000 (ESI and PF applicable)', async () => {
    ...
    const forms = await db.complianceForm.findMany({ where: { employeeId: empId } });
    ...
    const finalForms = await db.complianceForm.findMany({ where: { employeeId: empId } });
